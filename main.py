...
while flag_start:
      print("Команда: ", end = '')
      command = input()
      if command != "help" and command != "enter":
...
if command == "help":
      ...
      while flag_glob:
            ...
            while (flag != True) :
                  ...
                  cb = input()
                  if cb in names.brands: flag = True
                  else: print('данные введены некорректно, \nповторите ввод')
            print()
            ...
            while (flag != True):
                  ...
                  cm = input()
                  if cm in names.models_by_brand[cb]: flag = True
                  else: print('данные введены некорректно, \nповторите ввод')
            print()
            ...
            while (flag != True):
                  ...
                  chp = int(input())
                  if chp > 0 and chp < 2100: flag = True
                  else: print('данные введены некорректно, \nповторите ввод')
            print()
            ...
            key = input()
            if (key == 'да'):
                  print('идёт вычисление...')
                  flag_glob = False

      # переводим данные в dataframe
      order_data = pd.DataFrame(order_dict, index = [0])

      # приводим строковые данные к численному формату (как обучающие, так и пользовательские)
      label_encoder = LabelEncoder()
      data['car_brand'] = label_encoder.fit_transform(data['car_brand'])
      order_data['car_brand'] = label_encoder.transform(order_data['car_brand'])
      data['car_model'] = label_encoder.fit_transform(data['car_model'])
      order_data['car_model'] = label_encoder.transform(order_data['car_model'])
      data['car_city'] = label_encoder.fit_transform(data['car_city'])
      order_data['car_city'] = label_encoder.transform(order_data['car_city'])
      data['car_fuel'] = label_encoder.fit_transform(data['car_fuel'])
      order_data['car_fuel'] = label_encoder.transform(order_data['car_fuel'])
      data['car_transmission'] = label_encoder.fit_transform(data['car_transmission'])
      order_data['car_transmission'] = label_encoder.transform(order_data['car_transmission'])
      data['car_drive'] = label_encoder.fit_transform(data['car_drive'])
      order_data['car_drive'] = label_encoder.transform(order_data['car_drive'])

      # разделяем данные на факторы и отклики
      factors = data[['car_brand', 'car_model', 'car_city', 'car_fuel', 'car_transmission', 'car_drive', 'car_mileage', 'car_engine_capacity', 'car_engine_hp', 'car_age']]
      ...
      #print("точность регрессии случайным лесом, при размере обучающей выборки в 79%", regressor.score(f_test, r_test).round(3))
